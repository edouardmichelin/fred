...
class ActivityRepositoryImpl @Inject constructor(private val db: FirebaseFirestore) :
    ActivityRepository {
  ...
  override suspend fun getAllActivitiesOf(userId: String): Flow<State<List<Activity>>> = flow {
    ...
    try {
      val refs = collection.whereEqualTo(Location::ownerId.name, userId).get().await()
